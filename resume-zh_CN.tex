...
\contactInfo{tel: 15301959284}{mail: likai2023@126.com}{嵌入式软件工程师}{}
% {E-mail}{mobilephone}
...
\begin{itemize}  
	...
	\item \textbf{Android地面站核心功能开发}:  基于\textbf{MAVLink v2.0}协议实现飞控通信，解析HM30数传模块的实时数据（姿态、GPS、电池状态），封装控制指令。  多线程协同框架：主线程管理UI渲染及Mapbox路径显示；独立线程处理MAVLink协议解析/封装，保障飞控数据实时性； 异步线程拉取RK3588的RTSP视频流，实现视频流与飞控数据的帧同步叠加显示。开发蓝牙透传模块，连接HM30数传硬件，支持平板直接控制无人机启停、巡航模式切换。集成边缘计算结果：接收RK3588的YOLOv5s缺陷检测结果，在Mapbox地图动态标注异常位置，并触发无人机悬停指令。
...
\begin{itemize}
	...
	\begin{itemize}
		\item 基于图论理论，设计交流桩三环拓扑结构下的环形充电算法，优化充电模块功率分配，实现负载均衡。支持节点间充电路径动态调整，提升系统可靠性，目前处于功能测试阶段。
	\end{itemize}
\end{itemize}
